...
Aus dem **Repository-Root** (empfohlen):
...
Aus dem **Paketordner** `unified_ocr_app\` heraus (falls du direkt dort arbeitest):
...
py -3.10 unified_ocr_app\doctor.py --base-dir C:\OCR_Workdir
```

Vor einem Release:

1. Tests ausfuehren.
2. Lokalen Release-Check mit `py -3.10 unified_ocr_app\release_check.py` ausfuehren.
3. Doctor-Check mit `py -3.10 unified_ocr_app\doctor.py --base-dir C:\OCR_Workdir` auf dem Zielsystem ausfuehren.
